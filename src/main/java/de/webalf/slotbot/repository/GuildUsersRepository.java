...
public interface GuildUsersRepository extends JpaRepository<GuildUser, GuildUserId> {
	...
	@Query("SELECT g.id.userId FROM GuildUser g WHERE g.guild = :guild AND g.role = :role")
	Set<Long> findByGuildAndRole(@Param("guild") Guild guild, @Param("role") Role role);
	...
	List<GuildUser> deleteByGuildAndId_UserIdNotInAndRoleNot(Guild guild, Set<Long> userIds, Role role);
