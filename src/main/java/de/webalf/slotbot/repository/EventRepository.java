...
public interface EventRepository extends SuperIdEntityJpaRepository<Event> {
	...
	List<Event> findAllByGuildAndDateTimeBetween(@Param("ownerGuild") Guild ownerGuild, @Param("start") LocalDateTime start, @Param("end") LocalDateTime end);
	...
	List<Event> findAllByGuildAndDateTimeBetweenAndHiddenFalse(@Param("ownerGuild") Guild ownerGuild, @Param("start") LocalDateTime start, @Param("end") LocalDateTime end);
	...
	@Query(value = "SELECT e FROM Event e WHERE e.dateTime > :dateTime AND e.ownerGuild.id = :ownerGuild AND NOT EXISTS(SELECT di FROM EventDiscordInformation di WHERE di.event = e AND di.guild.id = :ownerGuild) ORDER BY e.dateTime")
	...
	@Query(value = "SELECT e FROM Event e WHERE e.dateTime > :dateTime AND e.shareable = true AND e.ownerGuild.id <> :ownerGuild AND NOT EXISTS(SELECT di FROM EventDiscordInformation di WHERE di.event = e AND di.guild.id = :ownerGuild) ORDER BY e.dateTime")
